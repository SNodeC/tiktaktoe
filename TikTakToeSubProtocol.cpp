...
TikTakToeSubProtocol::TikTakToeSubProtocol(const std::string &name,
                                           TikTakToeGameModel &gameModel)
    : web::websocket::server::SubProtocol(name), gameModel(gameModel),
      timer(net::timer::Timer::intervalTimer(
          [this]([[maybe_unused]] const void *arg,
                 [[maybe_unused]] const std::function<void()> &stop) -> void {
            this->sendPing();
            this->flyingPings++;
...
TikTakToeSubProtocol::~TikTakToeSubProtocol() { timer.cancel(); }
...
void TikTakToeSubProtocol::onMessageData(const char *junk,
                                         std::size_t junkLen) {
